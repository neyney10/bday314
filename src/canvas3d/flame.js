...
export function getFlameMaterial(isFrontSide) {
    ...
    return new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          pos.y *= 1. + (cos((posXZlen + 0.25) * 3.1415926) * 0.25 + noise(vec2(0, time)) * 0.125 + noise(vec2(position.x + time, position.z + time)) * 0.5) * position.y * 0.25; // flame height

          pos.x += noise(vec2(time * 2., (position.y - time) * 4.0)) * hValue * 0.0312 * 0.3; // flame trembling
          pos.z += noise(vec2((position.y - time) * 4.0, time * 2.)) * hValue * 0.0312 * 0.3; // flame trembling
